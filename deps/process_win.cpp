...
bool Process::try_get_exit_status(int &exit_status, unsigned long milliseconds) noexcept {
  if(data.id == 0)
    return false;
  ...
  DWORD wait_status = WaitForSingleObject(data.handle, milliseconds);
